...
const ChackoutFrom = ({ order }) => {
    ...
    const { _id, toolPrice, toolName, email } = order;
    console.log(toolPrice);
    // console.log(order);
    const price = Number(toolPrice)

    useEffect(() => {
        fetch('https://blooming-beyond-08690.herokuapp.com/create-payment-intent', {
            method: 'POST',
            headers: {
    ...
    const handleSubmit = async (event) => {
        ...
        if (intentError) {
            ...
            fetch(`https://blooming-beyond-08690.herokuapp.com/orders/${_id}`, {
                method: 'PATCH',
                headers: {
                    'content-type': 'application/json',
                    'authorization': `Bearer ${localStorage.getItem('accessToken')}`
